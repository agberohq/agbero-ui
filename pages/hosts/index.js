...
export default async function({ find, findAll, on, onUnmount, ready, inject }) {
    ...
    function sparkBarHtml(hostname){const spark=store.get('sparklines.'+hostname)||{},points=spark.p99||[];if(points.length<3)return'';const segs=points.map(v=>{const ratio=Math.min(v/500,1),hue=Math.round(120-ratio*120);return`<span class="spark-seg" style="background:hsl(${hue},65%,${ratio>0.5?'45':'50'}%);flex:1;min-width:3px;height:100%;border-radius:1px;opacity:.85;" title="${v>0?v.toFixed(0)+'ms':'idle'}"></span>`;}).join('');return`<div class="host-spark-bar" data-action="open-perf" data-hostname="${esc(hostname)}" style="display:flex;align-items:flex-end;gap:1px;height:18px;cursor:pointer;min-width:60px;">${segs}</div>`;}
    ...
    function hostCardHtml(hostname,hStats,cfgHost,term,certificates){
        ...
        const deleteBtn=isProtected?`<button class="btn small" disabled title="Cannot delete — protected host" style="opacity:0.4;cursor:not-allowed;">Delete</button>`:`<button class="btn small" data-action="delete-host" data-hostname="${esc(hostname)}" style="color:var(--danger);border-color:var(--danger);">Delete</button>`;
        const editBtn=isProtected?'':`<button class="btn small" data-action="edit-hcl" data-hostname="${esc(hostname)}">Edit HCL</button>`;
    ...
    on('[data-action="edit-hcl"]','click',async(e,btn)=>{e.stopPropagation();const h=btn?.dataset.hostname;if(!h)return;ui.btn.loading(btn,'…');const hcl=await api.getHostHCL(h);ui.btn.reset(btn);if(!hcl){notify.show('Could not load HCL — try again','error');return;}emit('host:open-edit-hcl',{domain:h,hcl});});

 ...
 */
import { listen, notify, modal, countdown } from '../lib/oja.full.esm.js';
...
export default async function({ find, on, onUnmount, ready, inject }) {
    ...
    function ruleRow(r) {
        ...
        const dateStr   = created && !isNaN(created) ? created.toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '—';
        const ip        = r.ip || '0.0.0.0';
        const expiresAt = (r.duration_sec && r.duration_sec > 0 && created) ? created.getTime() + r.duration_sec * 1000 : null;
    ...
    function render(data) {
        ...
        if (!tbody) return;
        if (!data) { tbody.innerHTML = emptyRow('⚠️ Firewall unavailable'); if (countEl) countEl.textContent = '—'; return; }
        if (data.enabled === false) { tbody.innerHTML = emptyRow('<span>🛡️ Firewall disabled</span><span>Enable in agbero.hcl to use this feature</span>'); if (countEl) countEl.textContent = '0'; return; }
        _rules = data.rules || [];
        if (countEl) countEl.textContent = String(_rules.length);
        if (!_rules.length) { tbody.innerHTML = emptyRow('<span>✅ No blocked IPs</span><span>All traffic is currently allowed</span>'); return; }
        tbody.innerHTML = sortRules(_rules).map(ruleRow).join('');
    }

    async function refresh() { render(await api.fetchFirewall()); _startCountdowns(); }
    ...
    on('.fw-unblock-btn', 'click', async (e, btn) => {
        ...
        if (btn.dataset.confirm === '1') {
            _confirmIp = null; btn.disabled = true; btn.textContent = '…';
            try {
                await api.deleteFirewallRule(ip);
                notify.show(`${ip} unblocked`, 'success'); refresh();
            } catch {
                btn.disabled = false; btn.textContent = 'Unblock';
                btn.className = 'btn small error fw-unblock-btn'; btn.dataset.confirm = '';
            ...
            _confirmIp = ip; btn.dataset.confirm = '1'; btn.textContent = 'Sure?';
            btn.className = 'btn small fw-unblock-btn'; btn.style.background = 'var(--warning)'; btn.style.color = '#fff'; btn.style.borderColor = 'var(--warning)';
            setTimeout(() => {
                if (btn.dataset.confirm === '1') { btn.dataset.confirm = ''; btn.textContent = 'Unblock'; btn.className = 'btn small error fw-unblock-btn'; btn.style.cssText = ''; _confirmIp = null; }
            }, 3000);
        }
    });
